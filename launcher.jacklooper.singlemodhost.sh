...
start()
{
	...
	# synth
	fluidsynth -s -i -a jack -m jack -g 2 fluidsynth/custom.sf2 -o synth.polyphony=1 > /tmp/llogs/fluidsynth.log 2>&1 &
	sleep 0.1
	# drum
	fluidsynth -s -i -a jack -m jack -g 2 fluidsynth/drum.sf2 -o synth.polyphony=5 -o shell.port=$((9800 + 1)) > /tmp/llogs/fluidsynth.log 2>&1 &
	sleep 0.1
	
	# wav2midi synth
	for i in $(seq 1 $MIDILOOPS)
	do
		fluidsynth -s -i -m jack -a jack -g 2 fluidsynth/custom.sf2 -o synth.polyphony=1 -o shell.port=$((9800 + $(($i+1)))) > /tmp/llogs/fluidsynth-loop$i.log 2>&1 &
		sleep 0.1
	done
	...
	python /home/flappix/docs/code/jacklooper/looper.py -l $LOOPS &
	#h2cli -s hydrogen/default.h2song -d jack > /tmp/llogs/hydrogen.log 2>&1 &
	...
	python MidiMap.py -c $(($FXLOOPS+1)) -cc 1 > /tmp/llogs/midimap.log 2>&1 &

	./midi_manager.jacklooper.singlemodhost.py -l $LOOPS -fx $FXLOOPS -m $MIDILOOPS
