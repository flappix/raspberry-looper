...
#define ADELAY_URI "http://lv2plug.in/plugins/bdelay"
...
int map_divisor (float div, int points[], int n)
{
	for (int i=n-1; i>=1; i--)
	{
		if ( div >= (points[i] + points[i-1]) / 2 )
		{
			return points[i];
		}
	}
...
run(LV2_Handle instance, uint32_t n_samples)
{
	...
	int points[4] = {4, 8, 12, 16};
	int new_divisor = map_divisor ( *(adelay->divisor),points, 4);
	...
	if (new_divisor != adelay->divisorold) {
		recalc = true;
	}
	...
	if (recalc) {
		...
		if (*(adelay->sync) > 0.5f && adelay->bpmvalid) {
			*(adelay->delaytime) = adelay->beatunit * 1000.f * 60.f / (adelay->bpm * new_divisor);
		} else {
